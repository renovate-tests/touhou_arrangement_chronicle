...
new Vue({
  ...
  mounted () {
    axios.get(`api/v1/artists?category=symbol`)
      .then(res => {
  ...
  methods: {
    setCategory: function(category, category_i18n) {
      ...
      if (this.category == 'symbol' || this.category == 'number' || this.category == 'kanji') {
        ...
        axios.get(`api/v1/artists?category=${category}`)
          .then(res => {
            this.artists = res.data;
          });
      } else if (this.category == 'alphabet') {
        this.detail_category = 'A';
        axios.get(`api/v1/artists?category=alphabet&detail_category=A`)
          .then(res => {
            this.artists = res.data;
          });
      } else if (this.category == 'kana') {
        this.detail_category = 'あ';
        axios.get(`api/v1/artists?category[]=hiragana&category[]=katakana&detail_category[]=あ&detail_category[]=い&detail_category[]=う&detail_category[]=え&detail_category[]=お`)
          .then(res => {
            this.artists = res.data;
          });
      } else {
    ...
    setDetailCategory: function(detail_category) {
      ...
      if (this.category == 'alphabet') {
        axios.get(`api/v1/artists?category=${this.category}&detail_category=${this.detail_category}`)
          .then(res => {
            this.artists = res.data;
          });
      } else if (this.category == 'kana') {
        ...
        axios.get(`api/v1/artists?category[]=hiragana&category[]=katakana&${params}`)
          .then(res => {
            this.artists = res.data;
          });
      }
  ...
  computed: {
    sortedArray() {
      return this.artists.sort((a, b) => a.name_ja > b.name_ja );
    },
  },
});
